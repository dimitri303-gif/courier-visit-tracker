...
### 6. `Settings` (Дистанційна конфігурація системи — Remote Config)
Усі параметри роботи додатка конфігуруються через цей лист без перевипуску APK:
* `points_version` (Версія списку точок для автооновлення на телефонах)
* `location_interval_ms` (Частота оновлення координат у фоні: `15000` мс)
* `distance_filter_m` (Поріг зміни локації для обробки: `10` м)
* `gps_accuracy` (Рівень точності GPS: `4` = High)
...
* `max_stay_minutes` (Максимальний час перебування на точці: `240` хв)
* `heartbeat_interval_minutes` (Частота фонового логування стану: `10` хв, наразі налаштовано на `1` хв)
* `exit_window_size` (Розмір ковзного вікна детекції виходу: `5` вимірів)
* `exit_threshold` (Поріг вимірів «ззовні» для фіксації виходу: `3`)
* `idle_threshold_minutes` (Мінімальний час на одному місці для реєстрації зупинки: `10` хв, наразі налаштовано на `5` хв)
* `idle_radius_m` (Максимальний радіус дрейфу GPS для зупинки: `20` м)
* `manual_checkin_enabled` (Увімкнути/вимкнути ручний чекін: `true`/`false`)
* `manual_checkin_max_distance_m` (Макс. відстань для відображення точок поруч: `200` м)
* `gps_single_timeout_ms` (Таймаут поодиноких GPS-запитів: `5000` мс)
* `sync_queue_max_size` (Максимальний розмір офлайн-черги: `5000` записів)
* `sync_backoff_initial_s` (Початкова затримка повтору синхронізації: `5` с)
* `sync_backoff_max_s` (Максимальна затримка повтору: `300` с)
* `http_timeout_ms` (Таймаут мережевих запитів: `30000` мс)
* `points_sync_interval_ms` (Інтервал синхронізації точок на мобільному: `30000` мс)
* `notification_title` (Заголовок сповіщення фонової служби)
* `notification_body` (Текст сповіщення фонової служби)
